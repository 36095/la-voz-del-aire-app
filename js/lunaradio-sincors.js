...
(function (d, D) {
  ...
  d.lunaradio = function (m, w) {
    ...
    function P() {
      ...
      b =
        'ie' == ka.browser.name.toLowerCase() &&
        12 > parseInt(ka.browser.version)
          ? !0
          : !1;
      b &&
            ...
            try {
              ...
              1 == k.results.length
                ? ((v = k.results[0].artworkUrl100),
                  ...
                  E('COVER: ' + v),
                  '' != Xa &&
                    (A = k.results[0].trackViewUrl + '&app=itunes&at=' + Xa),
                  z(v, A))
                : z(ma, '');
            } catch (L) {
              z(ma, '');
          ...
          try {
            ...
            if (k.icestats.source.length === D) v = k.icestats.source;
              ...
              for (var A = 0; A < k.icestats.source.length; A++) {
                ...
                Ca == L.substr(L.length - Ca.length, Ca.length) &&
      ...
      d.ajax({
        ...
        success: function (b) {
          try {
            ...
            d(b).find('track').find('title').text() !=
              d(b).find('track').find('artists').text() &&
    ...
    function Wa(b) {
      if ('true' == zb) {
        ...
        b &&
          ...
          d('#' + c + a + 'texttitle').html(
            d('.' + c + a + 'texttitlespan').first()
          ),
          d('#' + c + a + 'texttitle')
            .data('lunaradioMarquee')
            .pause());
        d('#' + c + a + 'textradioname').width() >
        d('.' + c + a + 'textradionamespan')
          .first()
          .width()
          ? d('#' + c + a + 'textradioname').hasClass(c + 'Marquee') &&
            (d('#' + c + a + 'textradioname').removeClass(c + 'Marquee'),
            d('#' + c + a + 'textradioname').html(
              d('.' + c + a + 'textradionamespan').first()
            ),
            d('#' + c + a + 'textradioname')
              .data('lunaradioMarquee')
              .pause())
    ...
    function Fb(b) {
      ...
      for (var h = a.length, f = b.length, k = 0, v, A = 0; A < h; A++)
        (v = a[A]), (k += v * v);
      a = Math.sqrt(k / h);
      ...
      if ('fake' == V || 'real' == V) {
        ...
        if ('fake' == V) {
          ...
          for (var b = 0; 511 > b; b += 1)
            R
              ? r.push(Math.floor((254 / (b / 100 + 1)) * Math.random() + 1))
              : r.push(0),
              (ya[b] += (r[b] - ya[b]) / 9);
          r = ya;
        }
        try {
          'real' == V &&
            ((r = new Uint8Array(ia.frequencyBinCount)),
            ia.getByteFrequencyData(r));
        } catch (xa) {}
        'animated' == Da &&
          d(
            '#' +
          ...
          switch (qa) {
            ...
            case 1:
              ...
              for (var a = 0; a < r.length / 2; a += 1)
                g.lineTo(
                  ((a * l.width) / r.length) * 2,
                  l.height - (r[a] * l.height) / 255 + 2
                );
              g.strokeStyle =
                'rgba(' + n(q).r + ', ' + n(q).g + ', ' + n(q).b + ', 0.5)';
              g.stroke();
    ...
    function ea(b, a, h, f, k, v) {
      ...
      if (v)
        for (b.beginPath(), v = 0; v < a.length - 1; v += 2)
          b.rect(a[v] - 2, a[v + 1] - 2, 4, 4);
    }
    ...
    function E(b) {
      if ('true' == Ib) {
        var a = new Date();
        b =
          a.getHours() + ':' + a.getMinutes() + ':' + a.getSeconds() + ': ' + b;
        window.console && console.log(b);
        0 < d('#debug').length &&
          d('#debug').html(d('#debug').html() + '<br>' + b);
      }
    }
  ...
  var m = {
      extend: function (e, p) {
        var y = {},
          z;
        for (z in e)
          y[z] = p[z] && 0 === p[z].length % 2 ? p[z].concat(e[z]) : e[z];
        return y;
      },
      has: function (e, p) {
        return 'string' === typeof e
          ? -1 !== p.toLowerCase().indexOf(e.toLowerCase())
          : !1;
      },
      lowerize: function (e) {
            ...
            for (Q = 0; Q < Ba.length; Q++)
              (S = na[++H]),
                ...
                'object' === typeof J && 0 < J.length
                  ? 2 == J.length
                    ? (this[J[0]] =
                        'function' == typeof J[1] ? J[1].call(this, S) : J[1])
                    : 3 == J.length
                    ? (this[J[0]] =
                    ...
                    : 4 == J.length &&
                      (this[J[0]] = S
                        ? J[3].call(this, S.replace(J[1], J[2]))
                        : D)
                  : (this[J] = S ? S : D);
        y += 2;
      }
